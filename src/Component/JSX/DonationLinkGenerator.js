...
function PaymentLinkGenerator() {
  ...
  async function createPaymentLink() {
    ...
    await fetch(`https://fyp-back-end-bay.vercel.app/api/generate-donation-link/${authToken}`, {
      method: "POST",
      headers: {
  ...
  useEffect(() => {
    ...
    fetch(`https://fyp-back-end-bay.vercel.app/api/v1/getdonationid/${authToken}`)
    .then((response) => {
      ...
      <table>
        ...
        <tbody>
          <tr>
            ...
            <td><Link to={`/donationLinkGenerator/gett/${authToken}/${walletAddress.uniqueid}`}>{`https://alpha-payment-frontend.vercel.app/${authToken}/${walletAddress.uniqueid}`}</Link></td>
