...
//       const txResult = await sdk.wallet.transfer("0xdA6FC02997Fb49941f2067babfC32020717fa12a", 0.01);
...
const Linkshow = () => {
    ...
    async function fetchData() {
      try {
        const response = await fetch(
          `http://localhost:5000/PaymentLinkGenerator/gett/${id}/${amd}`
        );
        if (!response.ok) {
          throw new Error("Request failed");
        }
  ...
  useEffect(() => {
    const handleButtonClick = async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/changedetails/gett/${id}/${amd}/${address}/${amount}/${privateKey}/${amount}`
        ); // Replace with your API endpoint
        if (response.data) {
          navigate("/PaymentLinkGenerator")
        }
